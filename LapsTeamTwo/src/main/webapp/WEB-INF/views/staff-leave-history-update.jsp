...
<form:form method="POST" commandName="leave"
	...
	<table class="table">
		<tbody>
			...
			<tr>
				<td>Start Date</td>
				<td><form:input size="20" path="startDate" type="date" id="datepicker1"/></td>
				<td><form:errors path="startDate" cssStyle="color: red;" /></td>
			</tr>
			<tr>
				<td>End Date</td>
				<td><form:input size="20" path="endDate" type="date" id="datepicker2"/></td>
				...
				<td colspan="5">
				 <form:select path="disseminationId">
				    <c:forEach var="staff" items="${staffMembers}">
				        <form:option
				            value="${staff.staffId}">${staff.name}</form:option></c:forEach>
				 </form:select>
				</td>
